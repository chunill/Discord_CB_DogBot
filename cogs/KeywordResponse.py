...
class KeywordResponse(BaseCog):
    ...
    async def on_message(self, message):
        ...
        if "好累" in message.content or "累了" in message.content:
            ...
            await message.channel.send(f"（{choice(sentences1) + choice(sentences2)}，看起來是想要安慰你）")
        
        if "不行" in message.content or "不可以" in message.content or "不乖" in message.content or "壞" in message.content:
            sentences = [
        ...
        if "卡哇" in message.content or "可愛" in message.content:
            sentences = [
